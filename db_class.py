...
class DB_Worker:
    ...
    def add_user(self, user_id, lang):
        self._sql_execute(f'INSERT INTO `users` VALUES ("{user_id}", "{lang}")', fletch=False)
    
    def first_seen(self, user_id):
        seen = self._sql_execute(f'SELECT COUNT(`lang`) FROM `users` WHERE `id` = "{user_id}"')
    ...
    def get_text(self, user_id, what):
        lang = self._sql_execute(f'SELECT `lang` FROM `users` WHERE `id` = "{user_id}"')

        return get_json(open(f'lang/{lang}.json', 'r', encoding='utf-8').read())[what]
